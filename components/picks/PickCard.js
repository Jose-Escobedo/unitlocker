...
import { Crosshair, CircleDot, Dribbble, Sword, BarChart2, Clock } from 'lucide-react';
...
function pctColor(str) {
  ...
  if (isNaN(n)) return '#f5f6f8';
  if (n >= 70) return '#00e5a0';
  if (n >= 55) return '#f5c842';
  return '#ff4757';
}

function StatsGrid({ stats, prediction }) {
  const COLS = [
  ...
  return (
    <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
      <div style={{
        ...
        {COLS.map((c, i) => (
          <div key={c.key} style={{
            padding: '6px 4px', textAlign: 'center', fontSize: 9.5, fontWeight: 500,
            letterSpacing: '0.07em', color: 'rgba(245,246,248,0.4)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${COLS.length}, 1fr)` }}>
        {COLS.map((c, i) => {
          ...
          if (isPct) {
            color = pctColor(val);
            tint = parseInt(val) >= 55;
          } else if (isDiff && typeof val === 'number') {
            const favorable = prediction === 'Over' ? val > 0 : val < 0;
            color = favorable ? '#00e5a0' : Math.abs(val) < 0.5 ? 'rgba(245,246,248,0.55)' : '#ff4757';
          }

          const display = isDiff && typeof val === 'number'
            ? (val > 0 ? `+${val}` : `${val}`)
            : val;

          return (
            <div key={c.key} style={{
              padding: '9px 4px', textAlign: 'center', fontSize: 13, fontWeight: 600,
              color, fontFamily: "'DM Mono', monospace",
              background: tint ? 'linear-gradient(180deg, rgba(0,229,160,0.1) 0%, rgba(0,229,160,0.03) 100%)' : 'transparent',
              borderRight: i < COLS.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
            }}>
              {display}
            </div>
...
export default function PickCard({ pick }) {
  ...
  const hasHistory = (pick.stats?.history?.length ?? 0) > 0;
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ display: 'inline-flex', gap: 6, flexWrap: 'wrap' }}>
            <span style={{
              ...
              padding: '4px 9px', borderRadius: 999, fontSize: 10.5, fontWeight: 600, letterSpacing: '0.08em',
              background: 'rgba(255,255,255,0.04)', color: 'rgba(245,246,248,0.78)',
              border: '1px solid rgba(255,255,255,0.06)', fontFamily: "'DM Mono', monospace",
            }}>
            ...
            {pick.isHot && (
              <span style={{
                ...
                border: '1px solid rgba(255,107,53,0.32)', boxShadow: '0 0 14px rgba(255,107,53,0.18)',
                fontFamily: "'DM Mono', monospace",
              }}>
                <FireIcon /> HOT
              </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 24, alignItems: 'flex-start' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 22, fontWeight: 700, letterSpacing: '-0.015em', color: '#f5f6f8', fontFamily: "'Inter', sans-serif" }}>
              {pick.playerName}
            </h3>
            <p style={{ margin: '4px 0 0', fontSize: 12.5, color: 'rgba(245,246,248,0.55)', display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: "'DM Sans', sans-serif" }}>
            ...
            {dateLabel && (
              <p style={{ margin: '5px 0 0', fontSize: 11, color: 'rgba(245,246,248,0.35)', display: 'flex', alignItems: 'center', gap: 5, fontFamily: "'DM Mono', monospace" }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, textAlign: 'right' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10.5, fontWeight: 600, color: predColor, letterSpacing: '0.1em', fontFamily: "'DM Mono', monospace" }}>
              <ArrowIcon direction={pick.prediction} />
              <span>{pick.prediction.toUpperCase()}</span>
            </div>
            <div style={{ fontSize: 32, fontWeight: 600, color: predColor, letterSpacing: '-0.015em', lineHeight: 1, textShadow: isOver ? '0 0 18px rgba(0,229,160,0.35)' : '0 0 18px rgba(255,71,87,0.35)', fontFamily: "'DM Mono', monospace" }}>
              {pick.line}
            </div>
        ...
        {/* Notes + Stats */}
        {(pick.notes || pick.stats) && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {pick.notes && (
              <div style={{ position: 'relative', padding: '10px 12px 10px 16px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10 }}>
                <div style={{ position: 'absolute', left: 0, top: 8, bottom: 8, width: 2, background: `linear-gradient(180deg, ${sport.color}, transparent)`, borderRadius: 2, boxShadow: `0 0 6px ${sport.glow}` }} />
        ...
        {hasHistory && (
          <button
            ...
            style={{
              ...
              border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, cursor: 'pointer',
              fontSize: 11, fontWeight: 600, letterSpacing: '0.1em',
              color: 'rgba(245,246,248,0.4)', fontFamily: "'DM Mono', monospace", transition: 'all .2s',
            }}
